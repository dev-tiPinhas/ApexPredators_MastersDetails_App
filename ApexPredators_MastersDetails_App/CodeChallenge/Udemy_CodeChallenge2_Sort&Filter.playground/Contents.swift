...
func sortAndFilter(_ stringArray: [String]) -> [String] {
    let setInvalidChars: Set<String> = ["a", "e", "i", "o", "u"]
    
    let filteredList = stringArray.filter {
        !setInvalidChars.contains($0.first?.lowercased() ?? "")
    }
    
    return filteredList.sorted(by: { $0.lowercased() > $1.lowercased() } )
}
